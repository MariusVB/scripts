...
netmask="$(printf "%s\n" "$ipmask" | cut -d "/" -f 2)"
netaddress="$(/usr/bin/sipcalc "$ipmask" | awk '/Network address/ {print $NF}')"
...
# Function for resetting wireless interface.
function reset_interface() {
  ...
  echo "Interface $interface reset to starting properties"
  exit 0
}
...
function main() {
  ...
  # Set found IP and MAC for wireless interface.
    while read -r hostline; do
      ...
      if [ "$routermac" != "$newmacset" ]; then
            ...
            if [[ $? -eq 0 ]]; then
              ...
              printf "%b\n" "Successfully hijacked IP: $newipset, MAC: $newmacset"
              exit 0
      fi
